...
说明：175 份 PDF 已作为样本池下载完成，并已进入完整结构化流程。本次完整流程对 175 条记录完成解析、抽取、校验、单位归一化和量化评分。
...
| `base_attention_score` | float | 单年度基础关注分 |
| `cross_year_pressure_score` | float | 同公司跨年恶化压力分 |
...
- 优先使用 MinerU markdown；如果没有对应 markdown，则使用 `pypdf` 从本地 PDF 抽取文本。
...
理由：
- 样本池扩展到 175 份 PDF。
- 175 条记录已完成结构化解析、抽取、校验、归一化和评分。
- 形成 342 条同公司跨年匹配事件，并将跨年恶化信号纳入关注清单排序。
...
| 风险 | 应对 |
|---|---|
| PDF 文件较大，仓库体积可能过大 | 不上传 PDF 原文，保留完整 metadata、下载脚本和解析样本 |
| 解析文本可能漏掉表格细节 | 保留 evidence、解析样本和人工评估材料，高关注记录人工复核 |

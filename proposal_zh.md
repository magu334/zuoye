...
如果一家公司在经营现金流偏弱、利润承压或流动性风险披露较强的情况下仍进行现金分红，投资者和分析师可能需要进一步复核其分红政策是否稳健。因此，本项目希望用公开年报文本和结构化财务字段，生成一个可追溯、可解释的关注清单。
...
说明：项目已将 PDF 样本池扩展到 175 份，并且 175 条记录均已完成解析、字段抽取、校验、单位归一化和量化评分。当前完整运行使用 `pypdf_text_fallback` 解析本地 PDF 文本；如果后续有 MinerU markdown，流程会优先使用 MinerU 文本。
...
```text
metadata -> PDF 下载 -> 数据审计 -> PDF 文本解析 -> Section Routing
...
2. 将 PDF 年报解析为可检索文本，优先使用 MinerU markdown，缺失时使用 `pypdf` fallback。
...
8. 用分红压力、现金流压力、利润压力和流动性风险披露构建 `base_attention_score`。
9. 生成同公司跨年匹配事件，并将跨年压力加入最终 `attention_score`。

## 六、核心改进点

### 1. 样本量扩展
原项目约 80 份 PDF，样本量偏小。当前已扩展到 175 份房地产上市公司年报 PDF，覆盖 2020-2024 年，满足 1.1 档 150+ PDF 的数据规模要求。
...
项目再根据同公司跨年匹配结果生成 `cross_year_pressure_score`，例如风险评分跳升、基础关注分跳升、分红上升但现金流下降、利润下降但仍分红等。
...
### 4. 跨年匹配
项目对同一公司的不同年份记录进行匹配，生成 342 条跨年事件，用于观察风险披露和现金流表现的变化。跨年事件不只是单独展示，还会转化为跨年压力分并进入最终关注清单排序。这比单年、单文档抽取更符合 1.1 档的复杂度要求。
...
| 风险 | 应对 |
|---|---|
| 年报 PDF 较大，GitHub 上传体积偏高 | 不上传 PDF 原文，保留完整 metadata、下载脚本和解析样本 |
| PDF 文本抽取可能遗漏复杂表格结构 | 对高关注记录人工复核，后续可补充 MinerU 表格解析 |
